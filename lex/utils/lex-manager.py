...
def read_bot_definition_file(file_name=BOT_DEFINITION_FILENAME):
    ...
    logger.info(
        'successfully read bot definition from file: {}'.format(file_name)
    )
...
def write_bot_definition_file(bot_definition, file_name=BOT_EXPORT_FILENAME):
    ...
    logger.info(
        'successfully wrote bot definition to file: {}'.format(file_name)
    )
...
def export_bot(bot_name=None, file_name=None):
    ...
    if file_name is None:
        print(
            json.dumps(bot_definition, indent=4, sort_keys=True, default=str)
        )
        write_bot_definition_file(bot_definition, BOT_EXPORT_FILENAME)
    else:
